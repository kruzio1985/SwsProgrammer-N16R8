# SWS Programmer — Telink TLSR825x programmer on ESP32-S3 (N16R8)
...
> **This repository ships the `N16R8` build:** `ESP32-S3-DevKitC-1-N16R8`
> with **16 MB (Quad SPI)** flash and **8 MB (Octal SPI)** PSRAM.
...
Prebuilt image: `firmware/sws_programmer_esp32s3_n16r8_v1.1.bin`.

**Via PlatformIO** (recommended — flashes bootloader + partition table + app):

```powershell
pio run -e esp32s3n16r8 -t upload --upload-port COMx
```

**Via esptool** (app image only, to a board that already has the Arduino partition table):

```powershell
esptool.py --chip esp32s3 --port COMx --baud 921600 write_flash -z 0x10000 firmware\sws_programmer_esp32s3_n16r8_v1.1.bin
```

### Build from source

```powershell
pio run -e esp32s3n16r8
```
...
Gotowy obraz: `firmware/sws_programmer_esp32s3_n16r8_v1.1.bin`.
